...
#更改颜色
def visualize_samples(pts, fig=None, bgcolor=(0, 0, 0), fgcolor=(0, 0, 1),
                  show_intensity=False, size=(800, 600), draw_origin=False):
    if not isinstance(pts, np.ndarray):
        pts = pts.cpu().numpy()
    if fig is None:
        fig = mlab.figure(figure=None, bgcolor=bgcolor, fgcolor=fgcolor, engine=None, size=size)
    ...
    if show_intensity:
        ...
        #s = 30 * torch.ones([pts.shape[0]])
        G = mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], mode='sphere', color=(0.9,0.1,0.39),
...
def draw_scenes(points, pts_samples, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    fig = visualize_samples(pts_samples, fig=fig)
...
def draw_corners3d(corners3d, fig, color=(1, 1, 1), line_width=3, cls=None, tag='', max_num=500, tube_radius=None):
    """
