...
class _PointnetSAModuleFSBase(nn.Module):
    ...
    def forward(self,
        ...
        :param boundary_scores: (B, N) tensor of confidence scores of points, required when using s-fps
        :return:
            new_xyz: (B, npoint, 3) tensor of the new features' xyz
            new_features: (B, npoint, \sum_k(mlps[k][-1])) tensor of the new_features descriptors
        """
...
#                     scores_1 = scores_slice.sigmoid() ** self.weight_gamma

#                     assert b_scores is not None
#                     scores_boundary = \
#                         b_scores[:, self.sample_range_list[i][0]:self.sample_range_list[i][1]].contiguous()
#                     scores_2 = scores_boundary.sigmoid() ** self.weight_gamma
# #求即使前景点，又是边缘点的概率
#                     scores_totol = scores_1 * scores_2
