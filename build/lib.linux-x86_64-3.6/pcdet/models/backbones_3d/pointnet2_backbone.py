...
from ...ops.knn import knn as boundary
...
class PointNet2FSMSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        self.SA_modules = nn.ModuleList()
        channel_in = input_channels - 3
        num_neighbor = self.model_cfg.SA_CONFIG.get('NUM_NEIGHBOR', 32)
        ratio = self.model_cfg.SA_CONFIG.get('RATIO', 0.6)       
        use_xyz = self.model_cfg.SA_CONFIG.get('USE_XYZ', True)
...
#加入boundary_scrose
        ...
        lables_pl= Variable(torch.randn(batch_size, npoints).type(dtype),requires_grad=False)
        g_xyz,g_lables,g_indx,_ = boundary.boundary_grouping(torch.unsqueeze(lables_pl,[-1]), num_neighbor, batch_dict, batch_dict, use_xyz)
        g_lables = torch.unsqueeze(g_lables,[-1])
        self_labels = torch.repeat(torch.unsqueeze(lables_pl, [-1]),[1, 1, num_neighbor])
        ...
        boundary_points = torch.gt(same_label_num, (num_neighbor * ratio))  #相似的标签大于某个值，则为边界点

        print("显示得到的边界点boundary_points="+boundary_points)
#通过mlp得到分数
       # boundary_scores = self.confidence_mlps[k]
     #  boundary_weight = 10 - 9 * boundary_points
        ...
        batch_dict['point_scores_list'] = b_scores_flatten
