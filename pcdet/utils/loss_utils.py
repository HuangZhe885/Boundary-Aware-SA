...
class PointSASALoss(nn.Module):
    ...
    def b_loss_forward(self, b_scores, b_labels):
        """
        Args:
            b_scores: List of points, [(N, 1): predicted point scores]
            b_labels: List of points, [(N, 1): assigned segmentation labels]
        Returns:
            b_loss: List of segmentation loss
        """
        b_loss = []
        for i in range(len(self.layer_weights)):
            bi_scores, bi_labels = b_scores[i], b_labels[i]
            if bi_scores is None or bi_labels is None:
                b_loss.append(None)
                continue

            positives, negatives = bi_labels > 0, bi_labels == 0
            cls_weights = positives * 1.0 + negatives * 1.0 # (N, 1)
            pos_normalizer = cls_weights.sum(dim=0).float()

            one_hot_targets = bi_scores.new_zeros(
                *list(bi_labels.shape), 2
            )
            one_hot_targets.scatter_(-1, (bi_labels > 0).long().unsqueeze(-1), 1.0)
            one_hot_targets = one_hot_targets[:, 1:] # (N, 1)

            bi_loss = self.loss_func(bi_scores[None],
                                     one_hot_targets[None],
                                     cls_weights.reshape(1, -1))
            bi_loss = self.layer_weights[i] * bi_loss.sum() / torch.clamp(
                pos_normalizer, min=1.0)
            b_loss.append(bi_loss)
