...
from ...ops.boundary import boundary
...
class PointNet2FSMSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        self.boundary_mlps = self.model_cfg.SA_CONFIG.get('BOUNDARY_MLPS', None)
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINT_LIST.__len__()):
            ...
            if self.boundary_mlps and self.boundary_mlps[k]:
                boundary_mlp = self.boundary_mlps[k].copy()
                if boundary_mlp.__len__() == 0:
                    boundary_mlp = None
            else:
                boundary_mlp = None
...
#边界分数 lable
        ...
        npoint = [4096,2048,1024,512]
        boundary_label = [None]
        for i in range(4):
            lables_pl= torch.randn(batch_size, npoint[i]).cuda()
            pointclouds_pl= torch.randn(batch_size, npoint[i],6).cuda()
            # global boundary_label
            boundary_label_i = boundary.get_boundary(lables_pl,pointclouds_pl)
            boundary_label_i = boundary_label_i.cuda()
            boundary_label.append(boundary_label_i[i])
            
        # _, npoint,_= xyz.size()
        # boundary_label = [None]
        # # npoint = 4096
        # lables_pl= torch.randn(batch_size, npoint).cuda()
        # pointclouds_pl= torch.randn(batch_size, npoint,6).cuda()

        # boundary_label = boundary.get_boundary(lables_pl,pointclouds_pl)
        # boundary_label = boundary_label.cuda()



        l_xyz, l_features, l_scores ,b_scores = [xyz], [features], [None],[None]
        for i in range(len(self.SA_modules)):
            li_xyz, li_features, li_scores ,bi_scores= self.SA_modules[i](
                l_xyz[i], l_features[i], l_scores=l_scores[i], b_scores=b_scores[i])
            l_xyz.append(li_xyz)
            l_features.append(li_features)
            l_scores.append(li_scores)
            b_scores.append(bi_scores)

        # prepare for confidence loss
        l_xyz_flatten, l_scores_flatten ,b_scores_flatten= [], [],[]
        ...
        for i in range(1, len(b_scores)):
            if b_scores[i] is None:
                b_scores_flatten.append(None)
            else:
                b_scores_flatten.append(b_scores[i].reshape(-1, 1))  # (N, 1)
        batch_dict['point_coords_list'] = l_xyz_flatten
        batch_dict['point_scores_list'] = l_scores_flatten
        batch_dict['boundary_label'] = boundary_label
        batch_dict['point_scores_b_list'] = b_scores_flatten
        ...
        batch_dict['point_l_scores'] = l_scores[-1]  # (B, N)
        batch_dict['point_b_scores'] = b_scores[-1]  # (B, N)
