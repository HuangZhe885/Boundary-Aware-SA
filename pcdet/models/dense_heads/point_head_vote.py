...
class PointHeadVote(PointHeadTemplate):
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        # _, point_features, _, boundary_label = self.SA_module(
        #     point_coords,
        #     point_features,
        #     new_xyz=vote_coords,
        # )
...
#insert boundary
            point_loss_b_sasa = 0
            point_loss_sasa_b_list = self.loss_point_sasa.b_loss_forward(
                self.forward_ret_dict['point_sasa_b_preds'],
                self.forward_ret_dict['point_sasa_b_labels']
            )

            for i in range(len(point_loss_sasa_b_list)):
                cur_point_loss_b_sasa = point_loss_sasa_b_list[i]
                if cur_point_loss_b_sasa is None:
                    continue
                point_loss_b_sasa = point_loss_b_sasa + cur_point_loss_b_sasa
                tb_dict['point_loss_b_sasa_layer_%d' % i] = point_loss_sasa_b_list[i].item()

            tb_dict['point_loss_sasa'] = point_loss_sasa.item()
            tb_dict['point_loss_b_sasa'] = point_loss_b_sasa.item()

            return point_loss_sasa,point_loss_b_sasa, tb_dict
        else:
    ...
    def get_loss(self, tb_dict=None):
        ...
        point_loss_sasa,point_loss_b_sasa, tb_dict_3 = self.get_sasa_layer_loss()
        ...
        if self.training:
            ...
            if self.enable_sasa:
                ...
                ret_dict.update({
                    ...
                    'point_sasa_b_preds': batch_dict['point_scores_b_list'],         ############insert          
                    'point_sasa_b_labels': batch_dict['boundary_label'],
                })
